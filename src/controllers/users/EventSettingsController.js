...
class EventSettingsController {
  async getSettings(req, res) {
    try {
      ...
      return res.status(500).json({ success: false, message: 'Erro ao buscar configurações do evento.' });
    }
  }
  ...
  async update(req, res) {
    try {
      const eventId = parseInt(req.params.event_id);
      const { show_gift_list, show_guest_messages, show_event_info, allow_guest_confirmation } = req.body;

      if (!eventId) {
        return res.status(400).json({ success: false, message: 'Evento não fornecido.' });
      }
